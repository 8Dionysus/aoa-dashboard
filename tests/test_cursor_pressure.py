...
class CursorRetentionTests(unittest.TestCase):
    ...
    def test_metadata_boundary_rejects_raw_and_nested_secret_content(self) -> None:
        ...
        nested_safe_field = copy.deepcopy(admitted["payload"])
        nested_safe_field["wake_observation"]["candidate_receipts"][0]["provenance"] = {
            "raw_owner_ref": "/bounded/nested.json"
        }
        with self.assertRaises(ValueError):
            make_observation(
                goal_id=GOAL_ID,
                master_thread_id=THREAD_ID,
                observation_id="nested-safe-field",
                entity_key="nested-safe-field",
                kind="correlation_envelope",
                payload=nested_safe_field,
                source_refs=[SOURCE_REF],
            )

        del_safe_field = copy.deepcopy(admitted["payload"])
        del_safe_field["wake_observation"]["candidate_receipts"][0]["raw_owner_ref"] = "/bounded/\x7f.json"
        with self.assertRaises(ValueError):
            make_observation(
                goal_id=GOAL_ID,
                master_thread_id=THREAD_ID,
                observation_id="del-safe-field",
                entity_key="del-safe-field",
                kind="correlation_envelope",
                payload=del_safe_field,
                source_refs=[SOURCE_REF],
            )
    ...
    def test_descriptor_close_reuse_and_guard_retarget_fail_before_both_writes(self) -> None:
        cases = ("close", "reuse", "same-inode-reuse", "same-ofd-reuse", "guard-close")
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
                ...
                with _ledger_lock(log_path, checkpoint_path) as lock_context:
                    ...
                    if case == "close":
                        ...
                        self.assertEqual(replacement_fd, record.lock_descriptor)
                    elif case == "same-ofd-reuse":
                        os.close(record.lock_descriptor)
                        os.dup2(record.lock_guard_descriptor, record.lock_descriptor)
                    else:
