    ...
    def walk(node: dict[str, Any], allowed: frozenset[str], path: str) -> None:
        for key, item in node.items():
            if not isinstance(key, str):
                ...
                continue
            if key == "error" and not _safe_metadata_field_context(key, path):
...
def _metadata_context_matches(path: str, context: str) -> bool:
    if path == context:
        return True
    if not path.startswith(context + "[") or not path.endswith("]"):
        return False
    index = path[len(context) + 1 : -1]
...
class _VerifiedLedgerLock:
    ...
    def __deepcopy__(self, memo: dict[int, Any]) -> _VerifiedLedgerLock:
        del memo
        raise TypeError("ledger lock capability cannot be copied")
...
class _LedgerCapabilityRecord(NamedTuple):
    ...
    lock_guard_descriptor_identity: tuple[int, int]
    lock_descriptor_flags: int
    lock_guard_descriptor_flags: int
...
def _set_lock_descriptor_flags(descriptor: int) -> int:
    """Require a lock descriptor to remain close-on-exec and return its flags."""
...
def _close_owned_lock_descriptors(
    ...
    guard_descriptor_identity: tuple[int, int],
    descriptor_flags: int,
    guard_descriptor_flags: int,
) -> None:
    """Close only descriptors still owned by this lock scope.

    The guard is an independent unlinked sentinel inode; a separate lease
    descriptor keeps the flock held while cleanup checks the primary.  If a
    descriptor number was externally closed and reused, the physical identity
    or the non-blocking flock check prevents cleanup from closing an unrelated
    descriptor.
    """
    ...
    if guard_descriptor != -1:
        try:
            ...
            current_flags = fcntl.fcntl(guard_descriptor, fcntl.F_GETFD)
            guard_valid = (
                guard_identity == guard_descriptor_identity
                and current_flags == guard_descriptor_flags
            )
        except OSError:
            pass
    if descriptor != -1:
        ...
        try:
            if descriptor_identity == (-1, -1):
                ...
                if current_identity == descriptor_identity:
                    ...
                    close_primary = current_flags == descriptor_flags
                elif guard_valid and current_identity == guard_descriptor_identity:
                    # The primary was retargeted to this scope's sentinel;
                    # it is safe to close the known replacement during
                    # cleanup after the write path has already failed closed.
                    close_primary = True
        except OSError:
            pass
        if close_primary:
...
def _ledger_lock(observation_log_path: Path, checkpoint_path: Path):
    ...
    guard_descriptor_identity = (-1, -1)
    descriptor_flags = -1
    guard_descriptor_flags = -1
    try:
        descriptor = os.open(lock_path, os.O_CREAT | os.O_RDWR, 0o600)
        fcntl.flock(descriptor, fcntl.LOCK_EX)
        descriptor_stat = os.fstat(descriptor)
        descriptor_identity = (descriptor_stat.st_dev, descriptor_stat.st_ino)
        descriptor_flags = _set_lock_descriptor_flags(descriptor)
        ...
        _set_lock_descriptor_flags(lease_descriptor)
        guard_descriptor, guard_path = tempfile.mkstemp(prefix=".aoa-dashboard-ledger-guard-")
        os.unlink(guard_path)
        guard_stat = os.fstat(guard_descriptor)
        guard_descriptor_identity = (guard_stat.st_dev, guard_stat.st_ino)
        guard_descriptor_flags = _set_lock_descriptor_flags(guard_descriptor)
        ...
        record = _LedgerCapabilityRecord(
            ...
            lock_guard_descriptor_identity=guard_descriptor_identity,
            lock_descriptor_flags=descriptor_flags,
            lock_guard_descriptor_flags=guard_descriptor_flags,
        ...
        _close_owned_lock_descriptors(
            ...
            guard_descriptor,
            descriptor_identity,
            guard_descriptor_identity,
            descriptor_flags,
            guard_descriptor_flags,
        )
        if lease_descriptor != -1:
            try:
...
def _require_verified_lock(
    ...
    try:
        ...
        descriptor_flags = fcntl.fcntl(record.lock_descriptor, fcntl.F_GETFD)
        if descriptor_flags != record.lock_descriptor_flags:
            raise ValueError("ledger OS lock descriptor flags changed")
        guard_stat = os.fstat(record.lock_guard_descriptor)
        guard_identity = (guard_stat.st_dev, guard_stat.st_ino)
        if guard_identity != record.lock_guard_descriptor_identity:
            raise ValueError("ledger OS lock guard identity changed")
        guard_flags = fcntl.fcntl(record.lock_guard_descriptor, fcntl.F_GETFD)
        if guard_flags != record.lock_guard_descriptor_flags:
            raise ValueError("ledger OS lock guard flags changed")
        # The independent sentinel detects retargeting to the guard itself;
        # an internal lease descriptor keeps the flock held if the primary
        # descriptor was externally closed and reused.
